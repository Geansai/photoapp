  ...
  def createOrder
   ...
   @cart = session[:cart] || {} # This gets the content from the current shopping cart
    ...
    @cart.each do |id, quantity| 
      ...
      stockphoto = Stockphoto.find_by_id(id)
      
      @orderitem = @order.orderitems.build(:stockphoto_id => stockphoto.id, :title => stockphoto.title, :description => stockphoto.description, :quantity => quantity, :price => stockphoto.price)
      @orderitem.save
    end
